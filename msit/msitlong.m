...
textSize = 36; % Size for instructions and block messages
stimSizeSmall = 64;
stimSizeLarge = 96;
...
instructions{2} = 'Your goal is to press the key on the response box\nthat corresponds to the number that differs.\n\nConsider the following examples...\n\nPress the SPACE bar to continue...';

slide1 = imread('Slide1.bmp');
slide2 = imread('Slide2.bmp');

instructions{3} = 'Press the spacebar to begin the practice trials';
instructions{4} = 'Press the spacebar to begin the experiment';
...
try
    ...
    Screen('TextSize',win,textSize);
    DrawFormattedText(win,instructions{3},'center','center',[255 255 255]);
    Screen('Flip',win);
    KbReleaseWait(-1);
    KbPressWait(-1);
    Screen('Flip',win);
    ...
    % Practice Trials
    % Pick one congruent and one incongruent, at random
    randomOne = randsample((find(blockType == 1)),1);
    randomTwo = randsample((find(blockType == 2)),1);
    whichBlocks = [randomOne randomTwo];
    whichBlocks = Shuffle(whichBlocks);
    randTrial = randi(trialsPerBlock);
    practiceTrials = actualTrials(whichBlocks,randTrial,:);
    for p = 1:2
        
        thisTarget = practiceTrials(p,1,1);
        thisTargetPosition = practiceTrials(p,1,2);
        thisDistractor = practiceTrials(p,1,3);
        thisTargetSize = practiceTrials(p,1,4);
        thisTrialDigits = [];
    ...
    Screen('TextSize',win,textSize);
    DrawFormattedText(win,instructions{4},'center','center',[255 255 255]);
    Screen('Flip',win);
    KbReleaseWait(-1);
    KbPressWait(-1);
    KbReleaseWait(-1);
